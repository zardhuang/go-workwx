...
> [!WARNING]
> 本库的 v2 版本仍可能发生不兼容的 API 变更，如字段名、方法名调整等等，请注意。在我们处理完成所有已知的边边角角之前，不会有 v2 版本的非测试版 tag。
>
> **我们不保证 v2 的任何两个测试版本 tags 之间的兼容性**，但我们会尽量为不兼容变更分别提供迁移指南。
>
...
*Update*: 自从这个库写出来，已经过了很久；截至 2019 年 8 月，已经有三四个同类项目了。
不过看了看这些“竞品”，发现自己这个库的类型设计、公开接口、access token 处理等方面还不赖。
为什么人们总是喜欢写死请求 `Host`、用全局量、为拆包而拆包甚至不惜公开内部方法呢？

*2026 Update*: **感谢大家仍然爱用本项目！**
作者换了工作之后，已经很久没有活跃维护本项目了，看着三位数的 forks 数量，深感惭愧。
但这也带来意想不到的好处——截至 2026 年 1 月，本仓库的绝大部分内容居然仍未被
[AI slop][ai-slop] 所污染！😎

[ai-slop]: https://en.wikipedia.org/wiki/AI\_slop

## Supported Go versions

本库的 v2.x 分支最低支持到 Go 的上一个稳定版本，当前（2026 年 1 月）为 go1.24。
...
* 严肃对待类型、公开接口
    ...
    - 不会越俎代庖，一言不合 `panic`。所有存量 `panic` 都仅存在于单元测试或本库测试命令中，不会影响通过正常途径调用本库的业务代码。
